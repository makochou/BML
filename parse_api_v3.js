...
for (const file of controllers) {
    ...
    // Updated regex to handle @PostMapping without parenthesis, e.g. @PostMapping or @PostMapping("/path")
    const methodRegex = /@(Get|Post|Put|Delete)Mapping(?:\(\s*(?:value\s*=)?\s*"([^"]*)"(?:[^)]*)\))?[\s\S]*?public\s+(?:<[^>]+>\s+)?([A-Za-z0-9<>_]+)\s+([A-Za-z0-9_]+)\s*\(([^)]*)\)/g;
        ...
        if (paramsStr.trim()) {
            ...
            for (let p of params) {
                ...
                if (parts) {
                    ...
                    if (pType.endsWith('DTO') || pType.endsWith('VO') || pType.endsWith('Body') || pType.endsWith('Command') || pType.endsWith('Query')) {
                        let innerType = pType.replace(/<.*>/, '');
