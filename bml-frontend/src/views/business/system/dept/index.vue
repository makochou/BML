<template>
  <div class="page-wrapper">
    ...
    <GovernanceListStage density="ultra" body-fill>
      <template #actions>
        <a-button type="primary" v-if="hasPermission('system:dept:add')" @click="handleAdd()">
      ...
      <a-table :key="tableResetKey" :data="pagedTableData" :loading="loading" :bordered="false" :pagination="false" row-key="id" :expanded-keys="expandedKeys" size="small" :scroll="{ x: '100%', y: '100%' }" :scrollbar="false" sticky-header :columns="visibleColumns" :column-resizable="{ mode: 'fixed' }" ref="tableRef" :style="tableStyle" :row-class="getRowClass" @row-click="handleRowClick" @column-resize="handleColumnResize" @row-dblclick="handleRowDblClick" @expand="handleExpandChange">
        ...
        <template #actions="{ record }">
          <div class="table-row-actions" @click.stop @dblclick.stop>
            <a-button type="primary" size="mini" class="table-action-btn table-action-btn--primary" v-if="hasPermission('system:dept:edit')" @click="handleEdit(record)">
              <template #icon><icon-edit /></template>
              编辑
            </a-button>
            <a-button size="mini" class="table-action-btn table-action-btn--danger" v-if="hasPermission('system:dept:remove')" @click="confirmDelete(record.id)">
        ...
        <a-tabs default-active-key="basic" size="small" class="form-tabs">
          <a-tab-pane key="basic" title="基本信息">
            <a-row :gutter="16">
              ...
              <a-col v-if="hasPermission('system:dept:field:parentId')" :span="12">
            ...
            <a-row :gutter="16">
              <a-col v-if="hasPermission('system:dept:field:deptName')" :span="12">
                <a-form-item field="deptName" label="部门名称">
                  <a-input v-model="formData.deptName" placeholder="请输入部门名称" />
                </a-form-item>
              </a-col>
              <a-col v-if="hasPermission('system:dept:field:deptCode')" :span="12">
                <a-form-item field="deptCode" label="部门编码">
            ...
            <a-row :gutter="16">
              <a-col v-if="hasPermission('system:dept:field:sort')" :span="12">
                <a-form-item field="sort" label="显示排序">
                  <a-input-number v-model="formData.sort" :min="0" placeholder="排序" style="width: 100%;" />
                </a-form-item>
              </a-col>
              <a-col v-if="hasPermission('system:dept:field:status')" :span="12">
...
const defaultColumns: BusinessTableColumn[] = [
  /* ── 核心标识（默认显示） ── */
  { key: 'deptName', title: '部门名称', dataIndex: 'deptName', width: 240, visible: true, fixed: 'left', ellipsis: true, sortable: true, titleSlotName: 'th-deptName', permission: 'system:dept:field:deptName' },
  { key: 'deptCode', title: '部门编码', dataIndex: 'deptCode', width: 140, visible: true, sortable: true, titleSlotName: 'th-deptCode', permission: 'system:dept:field:deptCode' },
